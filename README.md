...
4. **Fail-closed.** Missing policy, unknown tool, expired/missing capability, invalid/expired/consumed/binding-mismatched approval, PEP unavailable, kill, or suspend, parse failure → **DENY** + receipt; `gated_invoke` does not enter the tool. Halt mode can persist in a JSON file so a restarted process still denies.
...
| `docs/ROADMAP.md` | stub → v0.2 → v0.3 / EOI M1 → v0.3.1 approval binding |
